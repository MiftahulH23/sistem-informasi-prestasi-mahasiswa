                        ...
                        {pengajuanLomba.length > 0 ? (
                            pengajuanLomba.map((lomba, index) => (
                                <tr key={lomba.id} className="text-center">
                                    ...
                                    <td className="px-4 py-2">{lomba.judul_lomba}</td>
                                    <td className="px-4 py-2">{lomba.jenis_lomba}</td>
                                    <td className="px-4 py-2">
                                        {Array.isArray(lomba.anggota_kelompok) && lomba.anggota_kelompok.length > 0
                                            ? lomba.anggota_kelompok.join(", ")
                                            : "-"}
                                    </td>
                                    <td className="px-4 py-2">{lomba.dosen_pembimbing}</td>
                                    <td
                                    ...
                                    <td className="px-4 py-2 gap-2 grid place-items-center h-16">
                                        {reviewed[lomba.id] ? (
                                            ...
                                            <div className="flex gap-2 items-center justify-center">
                                                <button
                                                    onClick={() => updateStatus(lomba.id, "Diterima")}
                                                ...
                                                <button
                                                    onClick={() => updateStatus(lomba.id, "Ditolak")}
                            ...
                            <tr>
                                <td colSpan="8" className="text-center py-4">Tidak ada data pengajuan lomba.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
